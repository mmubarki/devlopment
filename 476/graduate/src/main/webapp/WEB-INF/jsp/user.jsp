        ...
        <c:choose>
            <c:when test="${user.getInterestedEvents() != null && 
                ...
                <c:forEach items="${user.getInterestedEvents()}" var="event">
                    <c:url var="viewEventUrl" value='/MVC/Events/view/${event.getId()}'/>
                    <li> <a href='${viewEventUrl}'> 
                            <c:out value="${event.getTitle()}"/> 
                            in : <c:out value="${event.getEventTimeStr()}"/>               
                            at : <c:out value="${event.getLocation()}"/>
                        </a> 
                        <c:url var="unlikeEventUrl" value='/MVC/Events/unlikeEvent/${user.getId()}/${event.getId()}'/>
                   ...
                   <c:url var="addToCalendarUrl" value="http://www.google.com/calendar/event">
                       ...
                       <c:param name="dates" ><c:out value='${LocalDateTime.parse(event.getEventTime().toString(), DateTimeFormatter.ofPattern("yyyyMMdd\'T\'HHmmss"))}/${LocalDateTime.parse(event.getEventTime().toString(), DateTimeFormatter.ofPattern("yyyyMMdd\'T\'HHmmss"))}'/></c:param> 
                  </c:url>
            ...
            <c:when test="${user.getCreatedEvents() != null}">
                ...
                <c:forEach items="${user.getCreatedEvents()}" var="event">
                    <c:url var="viewEventUrl" 
                           value='/MVC/Events/view/${event.getId()}'/>
                    <li>  <a href='${viewEventUrl}'> 
                            <c:out value="${event.getTitle()}"/> 
                            in : <c:out value="${event.getEventTimeStr()}"/>               
                            at : <c:out value="${event.getLocation()}"/>
                       ...
                       <c:param name="dates" >
                           <%--  
                           <c:out value='${LocalDateTime.parse(event.getEventTime().toString().DateTimeFormatter.ofPattern("yyyyMMdd\'T\'HHmmss"))}/${LocalDateTime.parse(event.getEventTime().toString(), DateTimeFormatter.ofPattern("yyyyMMdd\'T\'HHmmss"))}'/>
                       --%>
                       </c:param> 
                  </c:url>
                   &ensp;&ensp; <a href="${addToCalendarUrl}" target="_blank">
